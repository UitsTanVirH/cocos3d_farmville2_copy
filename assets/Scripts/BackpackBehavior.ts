...
export class BackpackBehavior extends Component {
  ...
  public Collect(objectNode: Node) {
    ...
    const endPointGetter = () => {
      const wp =
        behavior.ObjectType === ObjectType.COIN
          ? this.node.parent.getWorldPosition()
          : this.Backpack.worldPosition.clone();
    ...
    Mover.move(MoveType.ARC, {
      ...
      onComplete: () =>
        behavior.ObjectType === ObjectType.COIN
          ? this.onCoinCollected(objectNode)
          : this.OnCollected(objectNode),
    });
  }
  ...
  public GetCoin() {
    ...
    this.m_currentCoinCount--;
    this.node.emit(
      BackpackEvent.ITEM_REMOVED,
      coin.getComponent(BackPackObjectBehavior),
    );
    return coin;
  }
  ...
  public GetTopType() {
    if (this.m_stackedObjects.length === 0) return null;
    const b = this.m_stackedObjects[
      this.m_stackedObjects.length - 1
  ...
  public GetCoinCount() {
    return this.m_currentCoinCount;
  }
